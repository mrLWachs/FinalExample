...
public class GameEngine 
{
    ...
    public GameEngine(UI ui, JLabel heroImage, JLabel objectiveImage,
                      JLabel enemyImage, LinkedList<JLabel> wallImages) {
        objective = new Objective(objectiveImage);      // create objective
        walls = new LinkedList<>();                     // create all walls
        for (int i = 0; i < wallImages.size(); i++) {   // traverse the array
            walls.add(new Wall(wallImages.get(i)));     // create wall objects
        }
